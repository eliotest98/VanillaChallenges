...
public class MySql implements Database {
    ...
    public String insertDailyChallenges() {
        ArrayList<Challenge> challenges = MySql.instance.getAllChallenges();
        int count = 1;
        if (challenges.isEmpty()) {
            ...
            for (String key : keys) {
                ...
                MySql.instance.insertChallenge(challenge.getChallengeName(), challenge.getTimeChallenge());
                count++;
            }
            return nome;
        } else {
    ...
    public void loadPlayersPoints() {
        Main.instance.getDailyChallenge().setPlayers(MySql.instance.getAllChallengers());
    ...
    public void backupDb(int numberOfBackupedFile) {
        ...
        if (!configFile.exists()) {
            try {
                ...
                if (!folderCreate) {
                    if (folder.listFiles().length >= numberOfBackupedFile) {
