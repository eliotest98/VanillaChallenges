...
public class RightPage extends Callback {
    ...
    public void execute(InventoryClickEvent inventoryClickEvent, List<?> items) {
        ...
        String nameItemConfig = getInterface().getItemsConfig().get(
                slots.get(Integer.parseInt(getNbtList().getNbt("{positionItem}")))).getNameItemConfig();
